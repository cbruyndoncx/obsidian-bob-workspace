...
Install this folder and `bob-workspace-bootstrap/` in your agent's skills directory. No brncx-skills vault, context-pack skill, or other agent skill is required. The target vault needs BOB Workspace installed. Scripts in this folder use Python 3.11+ and `uv`; `recover_after_apply.py` also runs directly with `python3`.

Run `uv run scripts/...` commands with this skill folder as the current directory, or pass an absolute script path. Pass absolute paths for the target vault's `workspace.json` and candidate JSON files when working from the skill folder.

The installed plugin's active config is `<vault>/.obsidian/plugins/bob-workspace/workspace.json`. Bundled starter templates are inside `main.js`; custom template files can be added under the installed plugin's `templates/` folder. If the BOB source checkout is available, its editable built-in template files are under the repository's `templates/` folder.
  ...
  - id: create
    ...
    inputs:
      ...
      - name: base
        type: string
        required: false
        description: "Build from the current live workspace.json, an on-disk custom template, the vault's schema sources, or a bundled template source from the BOB repository if available. Built-in templates are compiled into main.js and do not appear as files in the installed plugin folder."
    outputs:
...
- **A custom template file MUST carry a `_template` block.** The switcher includes bundled templates automatically and lists on-disk custom `templates/*.json` only when they have `_template` metadata (`id`/`label`/`description`/`order`). Pick an order after the existing choices.
...
- **Switching templates archives outgoing assets.** On a change from one active template to another, BOB archives the old schema source, derived schema files, and Bases and records each move in a `template-switch-*.json` journal. It seeds the new template's assets and regenerates schema outputs when needed. If a referenced Base is missing afterward, use Settings → BOB Workspace → Generate missing Bases, or inspect the journal with `python3 scripts/recover_after_apply.py --vault <vault> --dry-run` and restore missing archived Bases. The helper does not run any vault-specific `regenerate.py` script.
- **Do not reuse a bundled template's file name for a custom template.** The five bundled names (`bob`, `cadence`, `crm`, `emai`, `minimal`) take precedence over on-disk files of the same name. Use a new `workspace-<id>.json` file.
...
- **`base-embed`/`base-link` need a real Base path.** Pass `"view": "X"` plus a top-level `bases[entity] = {"file": "path/to/X.base"}` mapping, or pass the full `"base": {"file": "path/to/X.base", "view": "X"}`. Resolve paths against the target vault; a default path may not exist there.
- **Bundled templates live in `main.js`.** On-disk files under the installed plugin's `templates/` folder add custom choices. Read the BOB source checkout's `templates/workspace-*.json` only when that checkout is available; otherwise use the current live workspace or the vault's schemas as the create route's base.
...
- **Use the plugin's schema regeneration.** `workspace.json` configures the UI. Schema source YAML lives in the configured schema folder, and the plugin's Data model → Regenerate action creates derived files. No vault-local regeneration script is required.
...
4. **Acceptance contract holds**: `scripts/validate_workspace.py` (render guard ON) reports 0 errors on the live file and any custom templates. When the BOB source checkout is available, check its bundled template sources after validator changes.
...
- **Scope boundary**: this skill composes `workspace.json` and custom workspace templates. The sibling bootstrap skill handles schema YAML, optional `.base` files, and coverage diagnostics. Plugin installation and note migration are separate tasks.
...
- **Technical**: `create` writes `<plugin>/templates/workspace-<id>.json` with a `_template` block (`id`/`label`/`description`/`order`). It must use a name distinct from all bundled and existing custom templates.
- **Technical**: validate the changed live or custom file. When the BOB source checkout is available, also validate its bundled template sources after changing the validator.
...
| `validate` | `validate_workspace.py` run against the target; errors reported first with JSON paths + fixes, then warnings; zero files written. |

## Related Skill

- [[bob-workspace-bootstrap]] — sibling skill for schema YAML, basic `.base` files, and workspace coverage diagnostics.
...
- [references/templates.md](references/templates.md) — bundled and custom template locations, with base choices for the create route
