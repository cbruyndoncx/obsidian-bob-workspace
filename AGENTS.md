...
- ID: `bob-workspace`
- Name: `BOB Workspace`
- Version: see `manifest.json` (authoritative — currently `0.14.4-bob.23`)
...
- `vendor/xlsx.mini.min.js` - SOURCE for the SheetJS (mini) library that is inlined into `main.js` (not loaded at runtime)
- `templates/workspace-*.json` - SOURCE for the workspace templates inlined into `main.js` (not loaded at runtime)

The workspace templates and the SheetJS XLSX library are **bundled into `main.js`**
(generated by `scripts/bundle-templates.js` and `scripts/bundle-xlsx.js`). Obsidian's
installer delivers only `main.js`/`manifest.json`/`styles.css`, so manual test installs
need only copy those three into `<vault>/.obsidian/plugins/bob-workspace/`.
...
- `vendor/xlsx.mini.min.js` and `vendor/xlsx.LICENSE` - SheetJS (mini) source + license; the library is inlined into `main.js` via `scripts/bundle-xlsx.js`.
...
- XLSX export/import uses the SheetJS (mini) library inlined into `main.js` via
  `loadBundledXLSX()`; exports default under `BOB Workspace/Exports`.
...
- Respect current responsive behavior and mobile limitations for interactive
  changes.
- Keep `vendor/xlsx.mini.min.js` in the repo and re-run `scripts/bundle-xlsx.js`
  after updating it, so the inlined XLSX library in `main.js` stays current while
  XLSX commands and settings remain available.
...
- Release assets only need `main.js`, `manifest.json`, and `styles.css`
  (plus `versions.json` for the store). Templates and the XLSX library are
  bundled into `main.js`; regenerate both bundles before tagging
  (`node scripts/bundle-templates.js && node scripts/bundle-xlsx.js`).
