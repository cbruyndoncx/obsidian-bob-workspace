...
**BOB Workspace** is an Obsidian plugin (forked from the Cadence plugin) providing a fully configurable GUI workspace layer over your markdown notes for CRM, deal pipelines, client work, finance/tax, suppliers & procurement, project management, daily planning, reminders, custom dashboards, and AI playbooks/skills — all backed by plain markdown.
...
- Internal identifiers are now BOB-branded: `BobPlugin`, `BobAppView`, `Bob*` classes, `bob-`/`bob-workspace-*` CSS classes and command IDs, and `VIEW_TYPE_BOB_APP` (value `bob-workspace-app`). Preserve the deliberate compatibility/attribution references: the **legacy `Cadence/…` fallback paths** (`WORKSPACE_CONFIG_PATH`, the schema-entity default-folder fallback), the shipped **Cadence Classic** template (`workspace-cadence.json`), and the **upstream credit** link to the original Cadence Planner plugin — do not rename these. Incidental legacy comments remain cleanup candidates; they are not compatibility contracts.
...
- **`src/`** — all plugin logic, one concern per module (originally migrated mechanically from the monolithic `main.js`; runtime behavior preserved):
  ...
  - `src/field-values.ts` — shared scalar conversion, structured-value guards, enum selection matching
  - `src/canvas-storage.ts` — serialized canvas/manifest persistence, validation, recovery snapshots and rollback
...
- Non-table Base views (board, calendar, cards) show an **Open Base** fallback by default. With `inlineNativeViews` enabled (Settings → App → Rendering), they mount a live Base embed through the internal embed registry with `#View` as the subpath. Table views keep the plugin's own editable inline table.
...
**Field UI types:** `text` (default), `email`, `number`, `currency`, `date`, `enum` (requires `options`), `tags`. Canonical schema shape is retained in `EntityField.schemaType`/`items`. Integers use validated numeric inputs, booleans use true/false selectors, and arrays with explicit string items use tags. Objects and other arrays use note editing.

**Structured-value safety:** `field-values.ts` owns scalar conversion, live-frontmatter write guards, and enum selection semantics. Both table and detail editors check structured values before choosing controls and again inside `processFrontMatter()`. Clearing a number removes the property; zero/false are preserved. Detail autosaves use per-field timers and flush on teardown. `fmtValue()` summarizes structured values before type formatting. XLSX uses marked JSON cells (`BOB:JSON:v1:`) for reversible arrays/objects and escapes prefix-shaped strings; blank imports leave existing properties untouched. See the [2026-09-07 review and resolution](docs/archive/2026-09-07-code-review.md).

**Mutate existing frontmatter via Obsidian's `processFrontMatter()`** — never string-replace existing YAML. New-note templates currently serialize initial frontmatter with `stringifyYaml()` before `vault.create()`:
...
Generic new-note defaults use `typeFilter || entityKey` and then overlay `typeFilters`, including its `type` and typed discriminator values. Explicit template frontmatter keys override those identity defaults; unrelated template keys do not remove them. **`entityKeyFromFile(app, file)`** is the reverse lookup (matches frontmatter `type:` first, then path prefix). Do not add a `typesFilter` option without implementing it first — it is not part of current `listEntityFiles()` behavior.
...
When schema support is enabled (top-level `workspace.json.schemas.enabled`, else `settings.useSchemas`), `applySchemas(app, settings)` reads YAML from the schema folder (default `00-CORE/Schemas/source`) and merges field/column/type/folder definitions into `ENTITIES` at runtime. Runtime mapping: canonical `type_value` (falling back to `schema.entity`) supplies `typeFilter`; `discriminator` supplies `typeFilters`; `location_pattern` supplies folder prefixes, and `fields` supplies form fields. Primary selection uses explicit `primary`, then the first non-`type` key field, then the first non-`type` field; columns default to the first ~5 fields. Explicit `bob.typeFilter`/`bob.typeFilters` override canonical identity. `person` maps to the `contact` runtime key without changing its note-facing type. The loader shares canonical source interfaces with the designer and reports YAML read/parse failures by path; full runtime schema validation remains separate from the designer validator.
...
- `templates/workspace-bob.json` embeds the BOB data model as schema YAML under `_assets.schemas`, written into a vault's schema folder for any file that is missing when the template is applied. Those bodies are **verbatim copies of the canonical `00-CORE/Schemas/source/<entity>.yaml` files** — never edit them in the template. After the canonical data model changes, run `npm run sync-schemas` (source: `--source DIR`, else `$BOB_CANONICAL_SCHEMAS`); `node scripts/sync-template-schemas.mjs --check` reports drift. `tests/template-schemas-canonical.test.js` fails on drift whenever `$BOB_CANONICAL_SCHEMAS` is set. A stale embedded copy is what seeded 74 outdated schema files into the BRNCX vault on 2026-09-17, when a sync that carried only markdown left the schema folder empty at template-apply time.
...
- **Template switching** — changing the template archives source schemas, derived outputs, and Bases from the configured folders. Before moves, a plugin-folder `template-switch-<template>-<timestamp>.json` records original workspace text/settings, the full move plan, external Base snapshots, and phase. Archive failures abort and roll back completed moves; later apply failures retain the archive and record the failed phase. External Bases stay in place because they may be shared. Successful application marks recovery `applied`. Re-applying the same template skips archiving, rewrites the workspace definition, and fills only missing assets. See [recovery instructions](docs/installing-into-existing-vault.md#recovering-a-failed-template-switch).

### Code style & development rules

- TypeScript in `src/`, bundled by esbuild (cjs, target es2021) into the committed `main.js`. The codebase is fully annotated: `noImplicitAny`, `noImplicitThis`, and `strictBindCallApply` are enforced; the shared domain model lives in `src/types.ts`. Explicit `any` is reserved for documented dynamic boundaries (frontmatter values, user-authored dashboard JSON, the untyped Bases API) — do not add new ones. `strict: false` leaves strictness gaps including `strictNullChecks`; migrate selected modules with a dedicated stricter check rather than assuming nullability is the only remaining work. Type-only changes must never alter runtime behavior. Prefer small, scoped edits in `src/`/`styles.css`.
...
`generateMissingBases(app, settings)` (command **"Generate missing bases"** / Settings → Data model → Bases) writes a `.base` for each known or schema-registered entity lacking one — a `filters` clause from the runtime entity's `typeFilters`/`typeFilter` or folder, and a `table` view whose `order` lists the columns (`file.name` for the primary field, bare field keys otherwise), with `properties.<id>.displayName` for readable headers. Missing-only: existing files are never overwritten. **In a Base, `order`/`sort` use bare property names (`person_category`), while `properties` keys and `filters` use the `note.<prop>` form; `file.name` is the primary field's column** — match this when hand-authoring or generating.
...
**New entity type lists nothing.** Check `entityFolder(key)` returns the correct path and the `primary` field is set; for schema entities confirm the schema folder and effective `typeFilter`/`typeFilters`.
