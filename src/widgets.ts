...
export function normalizeWidgetSourceConfig(source, fallbackEntityKey = null) {
  if (!source) {
  ...
  const baseConfig: any = source.base && typeof source.base === 'object' && !Array.isArray(source.base)
    ? {
...
export function normalizeWidgetSortSpec(sort) {
  if (!sort) return [];
...
export function filterEntitiesByBaseConfig(app, entityKey, entities, baseConfig, warnings: any[] = []) {
  ...
  if (baseConfig.typeFilters && typeof baseConfig.typeFilters === 'object') {
    filtered = filtered.filter((entity) => {
      ...
      return Object.entries<any>(baseConfig.typeFilters).every(([key, value]) => String(fm[key] ?? '') === String(value));
    });
  }
...
export async function resolveWidgetSource(app, source, fallbackEntityKey = null, settings: any = {}) {
