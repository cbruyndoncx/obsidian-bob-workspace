...
export let XLSX_LIB = null;
export function getXLSX(app) {
...
export function safeSheetName(raw, used = new Set<any>()) {
...
export function workbookEntityKeyFromSheet(sheetName) {
...
export function xlsxCellValue(value) {
...
export function entityRowsForWorkbook(app, entityKey) {
...
export function worksheetRowsForEntity(app, entityKey) {
  const def = ENTITIES[entityKey];
...
export async function writeWorkbookToVault(app, workbook, path) {
...
export function workbookExportFolder(settings: any = {}) {
...
export function entityKeysForWorkbookGroups(groupIds) {
...
export function selectedWorkbookEntityKeys(groupIds) {
  if (!groupIds || !groupIds.length) return [];
  return entityKeysForWorkbookGroups(groupIds);
}

export async function exportEntitiesXLSX(app, entityKeys, suffix = '', settings: any = {}) {
  const XLSX = getXLSX(app);
...
export async function exportAllEntitiesXLSX(app, settings: any = {}) {
...
export function normalizedImportHeader(value) {
  return String(value || '').toLowerCase().replace(/[^a-z0-9]/g, '');
}

export function configuredFieldAliases(def) {
  const aliases = {};
  Object.entries<any>(def?.fieldAliases || {}).forEach(([fieldKey, values]) => {
...
export function rowValueForField(row, field, def) {
...
export function normalizeImportValue(value, field) {
...
export async function importEntityRows(app, entityKey, rows) {
  const def = ENTITIES[entityKey];
...
export async function importWorkbookEntities(app, file) {
  const XLSX = getXLSX(app);
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    const nonEmptyRows = rows.filter((row) => Object.values<any>(row).some((v) => String(v || '').trim()));
...
export async function importWorkbookEntitiesFromBuffer(app, buffer, filename) {
  const XLSX = getXLSX(app);
...
export async function promptImportWorkbook(app, onDone: (...args: any[]) => any = () => {}) {
  ...
  const picker = new (class extends obsidian.SuggestModal<any> {
    files: any;
    onPick: any;
